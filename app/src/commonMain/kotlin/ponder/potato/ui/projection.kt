 ...
 */
data class Projection(val x: Float, val y: Float, val distance: Float)
...
fun projection(
    pointX: Float,
    pointY: Float,
    cameraHeight: Float = 8f,
    cameraDistance: Float = 8f,
    cameraRotationXDegrees: Float = 45f,
    ...
    // In a standard perspective projection setup (like OpenGL), the camera looks down the negative Z axis.
    ...
    if (cameraSpaceZ >= 0) {
        ...
        // Project onto the image plane. The distance to the projection plane is related to the focal length.
    ...
    return Projection(projectedX, projectedY, depth)
}